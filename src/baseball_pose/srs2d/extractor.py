...
mp_pose = SimpleNamespace(PoseLandmark=PoseLandmark)
...
def output_path(output_dir, suffix):
    return output_dir / f"{OUTPUT_PREFIX}_{suffix}"


def parse_args():
    parser = argparse.ArgumentParser(description="Extract stabilized 2D pose skeletons from a baseball video.")
    parser.add_argument("--input", type=Path, required=True, help="Input video path.")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--output-prefix", type=str, default=None, help="Optional output filename prefix. Defaults to input video name.")
...
def detect_video(input_path, args, static_image_mode, smooth_landmarks):
    pose_module = require_mediapipe_pose()
...
def require_mediapipe_pose():
    try:
        import mediapipe as mp
    except ModuleNotFoundError as exc:
        raise ModuleNotFoundError(
            "Jiaming's SRS 2D extractor requires mediapipe. Install project dependencies first."
        ) from exc
    ...
    try:
        ...
        raise ModuleNotFoundError(
            "The installed mediapipe package does not expose the classic Pose API "
            "(mediapipe.solutions.pose). Use a Python/MediaPipe version that supports it."
        ) from exc
    return pose_module
...
def write_videos(data, stable_xyz, stable_visibility, frame_quality, output_dir):
    ...
    raw_path = output_path(output_dir, "old_static_pose.mp4")
    stable_path = output_path(output_dir, "stable_pose.mp4")
    quality_path = output_path(output_dir, "stable_pose_quality_boxes.mp4")
    compare_path = output_path(output_dir, "raw_vs_stable.mp4")

    raw_writer = video_writer(raw_path, fps, width, height)
    stable_writer = video_writer(stable_path, fps, width, height)
    quality_writer = video_writer(quality_path, fps, width, height)
    compare_writer = video_writer(compare_path, fps, width * 2, height)
    ...
    for frame_idx, frame in enumerate(frames):
        ...
        compare_writer.write(np.hstack([raw, stable]))

    raw_writer.release()
    stable_writer.release()
    quality_writer.release()
...
def write_csv(data, accepted, reason, stable_xyz, stable_visibility, output_dir):
    path = output_path(output_dir, "pose_landmarks_stabilized.csv")
    width = data["width"]
    height = data["height"]
    fps = data["fps"]
...
def write_frame_quality_csv(data, frame_quality, output_dir):
    path = output_path(output_dir, "frame_quality.csv")
    fieldnames = [
        "frame",
        "time_sec",
        "quality_label",
...
def write_angle_csv(data, stable_xyz, output_dir):
    path = output_path(output_dir, "stable_angles.csv")
    with path.open("w", encoding="utf-8", newline="") as handle:
...
def write_contact_sheet(data, stable_xyz, stable_visibility, output_dir):
    ...
    path = output_path(output_dir, "raw_vs_stable_contact_sheet.jpg")
...
def write_quality_contact_sheet(data, stable_xyz, stable_visibility, frame_quality, output_dir):
    ...
    for i, thumb in enumerate(thumbs):
        ...
        x = pad + col * (320 + pad)
        sheet[y : y + thumb.shape[0], x : x + thumb.shape[1]] = thumb
    path = output_path(output_dir, "quality_boxes_contact_sheet.jpg")
    ok, encoded = cv2.imencode(".jpg", sheet)
    if not ok:
...
def write_report(input_path, data, accepted, reason, metrics, outputs, output_dir):
    ...
    report_path = output_path(output_dir, "stabilization_notes.md")
...
def main():
    ...
    args = parse_args()
    OUTPUT_PREFIX = args.output_prefix or args.input.stem
    ...
    summary_json = output_path(args.output_dir, "stabilization_summary.json")
    metrics = {
